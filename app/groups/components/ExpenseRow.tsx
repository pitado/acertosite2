...
function safeDateLabel(v: string | null | undefined) {
  if (!v) return "—";
  const d = new Date(v);
...
export function ExpenseRow({ e, refresh }: { e: Expense; refresh: () => Promise<void> }) {
  ...
  const dateLabel = useMemo(() => safeDateLabel(e.date_iso), [e.date_iso]);
  ...
  return (
    <li className="text-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="text-emerald-100/70 text-xs">• {dateLabel}</span>
